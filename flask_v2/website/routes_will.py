...
def add_exercise():
    ...
    if request.method == "POST":
        ...
        query_parameters = dict(name=form_package["name"][0], description=form_package["description"][0])
...
def delete_muscle():
    ...
    try:
        exercises = db.session.execute(sql)
        db.session.commit()
        flash(f"Deleted exercise-muscle relationships with muscle_id = {muscle_id}", category='success')
